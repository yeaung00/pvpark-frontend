...
export default function Search({ placeholder, handleSubmit, setInput }) {
  return (
    <form onSubmit={handleSubmit} className='flex flex-col w-full gap-3'>
      <div className='flex gap-3 relative'>
        <div className="absolute h-full flex items-center justify-center text-xl w-12"><FaSearch /></div>
        <input onChange={(e) => setInput(e.target.value)} className='pl-12 focus:outline-emerald-700 transition duration-200 flex-1 border-2 rounded-md p-2 text-xl' type="text" placeholder={placeholder} />
